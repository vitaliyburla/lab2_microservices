...
function App() {
    ...
    const reqToService = () => {
        setResp('');
        axios.get('http://192.168.64.12/api/service1').then((res) => {
            setResp(res.data);
        });
    };
    ...
    const reqToBrokenService = () => {
        setResp('');
        axios.get('http://192.168.64.12/api/service1/test').then((res) => {
            setResp(res.data);
        });
    };
    ...
    const req100ToBrokenService = () => {
        ...
        let date1 = new Date();
        for (let i = 0; i < 100; i++) {
            axios.get('http://192.168.64.12/api/service1/test');
        }
        let date2 = new Date();
    ...
    const sendMessageWithKafka = () => {
        ...
        axios
            .get(
                `http://192.168.64.12/api/service1/kafka-producer?message=${mes}`
            )
            .then((res) => {
                setResp(res.data);
            });
    ...
    const startKafkaConsumer1 = () => {
        setResp('');
        axios
            .get(`http://192.168.64.12/api/service2/kafka-consumer`)
            .then((res) => {
                setResp(res.data);
            });
    };
    ...
    const startKafkaConsumer2 = () => {
        setResp('');
        axios
            .get(`http://192.168.64.12/api/service3/kafka-consumer`)
            .then((res) => {
                setResp(res.data);
            });
    };
    ...
    return (
        <div className='App'>
            <header className='App-header'>
                ...
                <button onClick={() => sendMessageWithKafka()}>
                    Send Message
                </button>
                <p>Response: {resp}</p>
            </header>
